...
2. **Configure local environment**
   ...
   - Edit `ansible/.env.ansible` and set required variables:
     ...
     - `PROJECT_NAME` -- project name (default: `fastapi_app`), used for the app directory name
...
3. **Set up SSH access to server from local environment**
   ...
   **AWS EC2:** Use the key pair you selected (or created) when launching the instance.
   ...
   ```
   If you launched without a key pair, use EC2 Instance Connect to add your public key:
   ```bash
...
4. Runs the Ansible playbook:
   ...
   - Configures firewall and security (UFW, fail2ban, SSH hardening)
   - Sets up monitoring (Node Exporter)
   - Creates application directories
   - Authorizes the GitHub Actions SSH key
   - Clones the repository and runs initial deployment (if `GIT_REPO_URL` is set)
...
1. **Add GitHub secrets** (required for deployments)
   ...
   >
   > Project path defaults to `~/projects/fastapi_app`. If you changed `PROJECT_NAME`
...
2. **Verify `.env` on server** -- provisioning creates it from the template with generated secrets. Check that values are set:
   ```bash
   ssh -i <your-key> <user>@<server_ip> "grep -E 'SECRET_KEY|POSTGRES_PASSWORD' ~/projects/fastapi_app/.env"
   ```
